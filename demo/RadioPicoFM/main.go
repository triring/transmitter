// tinygo build -target=pico -size=short -o RadioPicoFM.uf2 .
...
func main() {
	...
	var CarrierWave uint32
	var IntermittentWave uint32
	CarrierWave = uint32(pwm.Top() / 2)       // 搬送波の設定
    if (pwm.Top() / 10) == 0 {
		IntermittentWave = 1
	} else {
		IntermittentWave = uint32(pwm.Top() / 10) // 断続波の設定
	}	// 以下はデバッグ用
