// tinygo build -target=pico -size=short -o NeeNaw.uf2 .
...
var Notes = []music.Note{
	{987.76660251, 0.5},	// nee	B5	
	{880.00000000, 0.5},	// naw	A5	
}
...
func main() {
	...
	var CarrierWave uint32 = uint32(tx.GetTop() / 2)       // 搬送波の設定
	var IntermittentWave uint32 = uint32(tx.GetTop() / 10) // 断続波の設定

	fmt.Printf("Transmitter Test\n")	
	for {
		// 定義している楽譜データを読み込み、1音づつ読み出し、再生していく。
		for _, n := range Notes {
